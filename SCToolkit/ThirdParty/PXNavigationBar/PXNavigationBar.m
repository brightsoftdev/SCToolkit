...
@synthesize navigationLevels = _navigationLevels;
...
- (void)adjustSubviews
{
    ...
    // The original layout - nav buttons were sit at the far right side of the nav bar view
...
//									   BUTTON_HEIGHT)];
    ...
    // For a better user experience, instead of letting users move mouse across a long distance from the back button, we put nav buttons just beside the back button
    [leftNavButton setFrame:NSMakeRect(SIDE_MARGIN * 2 + minBackButtonSize.width,
									   NSMidY(bounds)-(BUTTON_HEIGHT/2),
									   NAVBUTTON_WIDTH,
									   BUTTON_HEIGHT)];

	[rightNavButton setFrame:NSMakeRect(SIDE_MARGIN * 2 + minBackButtonSize.width + NAVBUTTON_WIDTH,
                                        NSMidY(bounds)-(BUTTON_HEIGHT/2),
                                        NAVBUTTON_WIDTH,
                                        BUTTON_HEIGHT)];
